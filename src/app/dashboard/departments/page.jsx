"use client";

import { useState, useEffect } from "react";
import { MdEdit, MdDelete } from "react-icons/md";
import { BeatLoader } from "react-spinners";
import { DataGrid } from "@mui/x-data-grid";
import { Button, Popover, Typography, Box, Paper } from "@mui/material";
import toast, { Toaster } from "react-hot-toast";
import DepartmentFormPopup from "@/components/dashboard/department-create-form/DepartmentForm";
import { BASE_URL } from "@/services/baseUrl";

const Departments = () => {
  const [departments, setDepartments] = useState([]);
  const [total, setTotal] = useState(0);
  const [page, setPage] = useState(0);
  const [limit] = useState(100);
  const [searchQuery, setSearchQuery] = useState("");
  const [loading, setLoading] = useState(false);
  const [fetchError, setFetchError] = useState(null);
  const [anchorEl, setAnchorEl] = useState(null);
  const [departmentToDelete, setDepartmentToDelete] = useState(null);
  const [openDialog, setOpenDialog] = useState(false);
  const [editDepartment, setEditDepartment] = useState(null);

  const columns = [
    { field: "name", headerName: "Department Name", width: 200 },
    { field: "parentName", headerName: "Parent Department", width: 200 },
    { field: "departmentHeadName", headerName: "Department Head", width: 200 },
    { field: "description", headerName: "Description", width: 300 },
    {
      field: "edit",
      headerName: "Edit",
      width: 80,
      sortable: false,
      renderCell: (params) => (
        <button
          onClick={() => handleOpenEditDialog(params.row)}
          aria-label="Edit department"
        >
          <MdEdit className="w-5 h-5 text-gray-500" />
        </button>
      ),
    },
    {
      field: "delete",
      headerName: "Delete",
      width: 80,
      sortable: false,
      renderCell: (params) => (
        <button
          onClick={(event) => handleOpenDeletePopover(event, params.row)}
          aria-label="Delete department"
        >
          <MdDelete className="w-5 h-5 text-red-500" />
        </button>
      ),
    },
  ];

  const handleOpenAddDialog = () => {
    setEditDepartment(null);
    setOpenDialog(true);
  };

  const handleOpenEditDialog = (department) => {
    setEditDepartment(department);
    setOpenDialog(true);
  };

  const handleCloseDialog = () => {
    setOpenDialog(false);
    setEditDepartment(null);
  };

  const handleSuccess = () => {
    setOpenDialog(false);
    setEditDepartment(null);
    fetchDepartments(page, searchQuery);
  };

  const handleOpenDeletePopover = (event, department) => {
    setAnchorEl(event.currentTarget);
    setDepartmentToDelete(department);
  };

  const handleCloseDeletePopover = () => {
    setAnchorEl(null);
    setDepartmentToDelete(null);
  };

  const handleDeleteDepartment = async () => {
    if (!departmentToDelete) return;
    handleCloseDeletePopover();
    try {
      setLoading(true);
      setFetchError(null);

      const response = await fetch(
        `${BASE_URL}/api/department/delete/${departmentToDelete.id}`,
        { method: "DELETE" }
      );

      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to delete department");
      }

      const data = await response.json();
      toast.success(data.message || "Department deleted successfully!", {
        position: "top-right",
      });

      await fetchDepartments(page, searchQuery);
    } catch (error) {
      console.error("Failed to delete department:", error);
      setFetchError(
        error.message || "Failed to delete department. Please try again."
      );
      toast.error(error.message || "Failed to delete department.", {
        position: "top-right",
      });
      handleCloseDeletePopover();
    } finally {
      setLoading(false);
    }
  };

  const fetchDepartments = async (pageNum, search = "") => {
    try {
      setLoading(true);
      setFetchError(null);
      const query = new URLSearchParams({
        page: (pageNum + 1).toString(),
        limit: limit.toString(),
        keyword: search,
      }).toString();

      const response = await fetch(`${BASE_URL}/api/department/list?${query}`);
      if (!response.ok) {
        throw new Error("Failed to fetch departments");
      }

      const data = await response.json();
      const departments = (data.data?.departments || [])
        .filter((dep) => dep && typeof dep === "object" && dep.id)
        .map((dep) => ({
          ...dep,
          description: dep.description || "-",
          parentName: dep.parent?.name || "-",
          departmentHeadName: dep.departmentHead?.name || "-",
        }));

      if (departments.length !== data.data?.departments?.length) {
        console.warn(
          "Filtered out invalid department entries:",
          data.data?.departments
        );
      }

      setDepartments(departments);
      setTotal(data.data?.total || 0);
    } catch (error) {
      console.error("Failed to fetch departments:", error);
      setFetchError("Failed to load departments. Please try again.");
      setDepartments([]);
      setTotal(0);
    } finally {
      setLoading(false);
    }
  };

  useEffect(() => {
    fetchDepartments(page, searchQuery);
  }, [page, searchQuery]);

  const handleSearchChange = (e) => {
    const query = e.target.value;
    setSearchQuery(query);
    setPage(0);
  };

  const CustomNoRowsOverlay = () => (
    <Box sx={{ p: 2, textAlign: "center", color: "gray" }}>
      No emergency contacts found
    </Box>
  );

  return (
    <div className="min-h-screen bg-white p-4">
      <Toaster position="top-right" reverseOrder={true} />
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-xl font-semibold text-gray-800">
          Departments ({total})
        </h1>
        <button
          onClick={handleOpenAddDialog}
          className="bg-[rgb(42,196,171)] text-white px-4 py-2 rounded-md flex items-center space-x-2"
        >
          <span>+ Add Department</span>
        </button>
      </div>

      <div className="flex flex-col md:flex-row space-y-4 md:space-y-0 md:space-x-4 mb-4">
        <input
          type="text"
          placeholder="Search Departments"
          value={searchQuery}
          onChange={handleSearchChange}
          className="border border-gray-300 rounded-md px-3 py-2 w-full md:w-1/4 focus:outline-none focus:ring-1 focus:ring-[rgb(42,196,171)]"
        />
      </div>

      {loading ? (
        <div className="flex justify-center items-center h-64">
          <BeatLoader color="#2ac4ab" height={50} width={5} />
        </div>
      ) : fetchError ? (
        <div className="text-center text-red-600 py-10">{fetchError}</div>
      ) : (
        <>
          <Paper sx={{ width: "100%", boxShadow: "none" }}>
            <DataGrid
              rows={departments}
              columns={columns}
              autoHeight
              initialState={{
                pagination: { paginationModel: { page, pageSize: limit } },
              }}
              pagination
              paginationMode="server"
              rowCount={total}
              onPaginationModelChange={(newModel) => setPage(newModel.page)}
              pageSizeOptions={[]}
              sx={{
                border: 0,
                boxShadow: "none",
                "& .MuiDataGrid-row.Mui-selected": {
                  backgroundColor: "rgba(234, 248, 244, 1)",
                  "&:hover": {
                    backgroundColor: "rgba(234, 248, 244, 1)",
                  },
                },
                "& .MuiDataGrid-cell": {
                  border: "none",
                },
                "& .MuiDataGrid-footerContainer": {
                  borderTop: "none",
                  borderBottom: "none",
                },
                "& .MuiDataGrid-columnHeader:focus, & .MuiDataGrid-columnHeader:focus-within, & .MuiDataGrid-columnHeader--sorted":
                  {
                    outline: "none",
                  },
                "& .MuiDataGrid-cell:focus, & .MuiDataGrid-cell:focus-within, & .MuiDataGrid-cell--sorted":
                  {
                    outline: "none",
                  },
              }}
              slots={{
                noRowsOverlay: CustomNoRowsOverlay,
              }}
              slotProps={{
                pagination: {
                  showrowsperpage: false.toString(),
                },
              }}
            />
          </Paper>
        </>
      )}

      <DepartmentFormPopup
        open={openDialog}
        onClose={handleCloseDialog}
        onSuccess={handleSuccess}
        department={editDepartment}
      />

      <Popover
        open={Boolean(anchorEl)}
        anchorEl={anchorEl}
        onClose={handleCloseDeletePopover}
        anchorOrigin={{ vertical: "bottom", horizontal: "center" }}
        transformOrigin={{ vertical: "top", horizontal: "center" }}
      >
        <Box sx={{ p: 2, width: 260 }}>
          <Typography sx={{ mb: 2 }}>
            Are you sure you want to delete{" "}
            <strong>{departmentToDelete?.name}</strong>?
          </Typography>
          <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1 }}>
            <Button
              variant="outlined"
              size="small"
              onClick={handleCloseDeletePopover}
              sx={{
                borderColor: "#ef5350",
                color: "#ef5350",
                "&:hover": { borderColor: "#e53935", color: "#e53935" },
              }}
            >
              Cancel
            </Button>
            <Button
              variant="contained"
              size="small"
              color="error"
              onClick={handleDeleteDepartment}
              disabled={loading}
            >
              {loading ? <BeatLoader color="#fff" size={8} /> : "Delete"}
            </Button>
          </Box>
        </Box>
      </Popover>
    </div>
  );
};

export default Departments;
