...
import PeerFeedbackFormPopup from "@/components/dashboard/peer-feedback-form/PeerFeedbackForm";
...
const PeerFeedback = () => {
  ...
  const handleOpenDeletePopover = (event, feedback) => {
  ...
  const handleDeleteFeedback = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/api/peer-feedback/delete/${feedbackToDelete.id}`,
        { method: "DELETE" }
      );
      ...
      await fetchFeedbacks(page, searchQuery);
    } catch (error) {
  ...
  const fetchFeedbacks = async (pageNum, search = "") => {
    try {
      setLoading(true);
      setFetchError(null);
      ...
      const response = await fetch(
        `${BASE_URL}/api/peer-feedback/list?${query}`
      );
      if (!response.ok) {
      ...
      const data = await response.json();
      const feedbacks = (data.data?.peerFeedbacks || [])
        .filter(
          (feedback) => feedback && typeof feedback === "object" && feedback.id
        )
        .map((feedback) => ({
          ...feedback,
          feedback: feedback.feedback || "-",
          providedByName: feedback.providedBy?.name || "-",
          providedToName: feedback.providedTo?.name || "-",
          providedBy: feedback.providedBy || { name: "-" },
          providedTo: feedback.providedTo || { name: "-" },
        }));

      if (feedbacks.length !== data.data?.peerFeedbacks?.length) {
        console.warn(
          "Filtered out invalid feedback entries:",
          data.data?.peerFeedbacks
        );
      }
      ...
      setFetchError("Failed to load feedbacks. Please try again.");
      setFeedbacks([]);
      setTotal(0);
    } finally {
      setLoading(false);
